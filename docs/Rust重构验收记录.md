...
> **2026-09-07 审查更正：下文 M0-M6 是此前实现方登记的历史记录，不是复核通过证明。撤回其中“全部达成”“双向兼容通过”“流式下载已验收”“内存达标”“回滚可靠”等超出测试证据的结论。M1-M6 均重新开放验收。**
> 三项 backup_integration 测试不等于 Python/Rust 双向恢复、HTTP 故障注入和进程崩溃恢复均已测试；独立 bench_d1 不等于完整服务内存。当前真实状态以文末修复记录和《Rust内存基准报告》为准。
...
M6 复核状态：未验收。完整服务容量、双向恢复、目标系统故障注入均须在上线前完成，不能留到上线后补做。

## 审查修复记录（2026-09-07）

- `6bf6f38`：禁止密码写入审计；启动先检查恢复标记；回滚失败保留现场。startup/backup 共 5 项测试通过。
- `fad8d02`：HTTP 存储互斥、独立工作者生命周期、备份按 64KiB 流式下载、临时文件自动清理、上传真实字节上限、向量内容校验、发布文件回滚、全文领域标签。startup/backup/ingest 共 13 项测试通过。
- `16b56d6`：对话单锁队列、背压、断连取消、中文安全截取、UTF-8/SSE 解析、上游限额、忽略不可信 XFF、测试夹具隔离。agent/api/stream 共 16 项测试通过。

以上命令均在本机 WSL 离线依赖下运行，模型调用使用本地模拟服务。没有访问正式数据或付费模型，没有完成目标机器容量验收。历史审计和旧备份若已包含初始管理员密码，修复代码不会清除旧副本；管理员须检查访问范围、换密并按保留策略处理旧备份，不能仅靠升级消除已有泄露。

## 部署数据布局

服务模板要求 `.env` 显式指定 `DATA_DIR=/var/lib/campus-policy-agent/data`，父目录由 `StateDirectory=campus-policy-agent` 提供专用写权限，供上传、恢复标记和回滚目录使用。代码和前端仍位于只读的 `/opt/campus-policy-agent`。

原 Python 常见路径为 `/opt/campus-policy-agent/backend/data`。切换前停写、制作并验证备份，经管理员确认后完整复制原数据到新位置并调整 www-data 所有权；保留原目录以便回滚。禁止直接指向空目录启动冒充迁移成功。模板 `ExecStartPre` 要求指定目录已有 campus.db；全新安装须单独显式初始化。不要将含密钥的 .env 或任何副本加入 Git。

登录限流默认使用 TCP peer 地址，不信任客户端 X-Forwarded-For。通过反向代理访问时默认共用代理地址桶，应由反向代理再施加真实客户端限流；不要以重新信任任意转发头的方式绕过限制。

## P1/P2 修复对照与最终本机验证

| 审查项 | 修复与证据 |
| --- | --- |
| 1 初始密码写入审计 | 审计改固定文字；真实子进程初始化后查询 audit_log，检查 stdout/stderr 不含测试密码 |
| 2 启动忽略恢复标记 | 打开数据库前拒绝未完成恢复；子进程验证不创建空库 |
| 3 回滚失败清除标记 | 检查数据库关闭/重开结果，逐步回滚；缺失回滚目录保留标记，无效新库可回退原库；Linux 同步目录元数据 |
| 4 备份整包读入内存 | 按 64KiB 流式下载，临时文件随响应释放；原文件下载同样改流式 |
| 5 存储并发与取消保护 | 管理存储操作统一互斥，冲突 409；独立工作者持有许可，HTTP drop 不提前解锁；维护请求计数由 RAII 管理 |
| 6 发布文件失败回滚 | 新文件跟随数据库事务成功提交；失败删除新增文件、保留草稿，已有同名文件校验内容；导入失败同样回滚 |
| 7 上传 2MiB 限制与吞错误 | 分路由真实字节限制，完整消费 multipart 并传播读取错误；约 2.4MiB 合法包 HTTP 上传发布通过，chunked 超限 JSON 413 |
| 8 SSE 丢事件与预算 | await 背压，64 事件/1MiB 总队列；上游单事件 1MiB、每次模型流/上下文 8MiB、回答 256KiB、每轮 8 工具/192 证据；5000 突发事件完整接收 |
| 9 断连未取消 | 监听接收端关闭，丢弃 runner 和上游请求后释放运行槽；断连回归通过 |
| 10 多锁死锁 | 单一队列状态锁，锁外等待和发送；取消、排队、panic 清理回归通过 |
| 11 中文乱码与切片 panic | UTF-8 按完整行解码，支持 CRLF/多行 data，拒绝坏 JSON/截断；输入与截断按字符处理 |
| 12 全文领域标签丢失 | 空 section_ids 写入文档级 NULL 标签；上传、发布、下载备份后 SQL 校验标签存在 |
| 13 包内容校验不足 | 校验有限值/非零归一化、C/Fortran、精确 NPY 长度、维度与溢出；文本 4MiB、精确条目白名单和临时目录自动清理 |
| 14 XFF 绕过登录限流 | 使用 TCP peer，不信任转发头；12 次伪造不同 XFF 的测试最后两次返回 429；PBKDF2 有界工作线程 |
| 15 部署路径与权限 | 专用 StateDirectory 可写父目录，DATA_DIR 显式配置，启动要求已有数据库；不自动迁移或修改正式数据 |
| 16 假指标与过度声明 | 实际数据盘容量、递归数据体积、累计 CPU 秒；Threads 不乘 1024；撤回独立检索 RSS 对完整服务的达标判定 |

本机 WSL 最终验证命令（均为本地操作，无付费模型）：

- `cargo test --locked --offline`：52 passed（19 单元、6 Agent、9 API、3 Backup、8 Ingest、2 Startup、5 Stream）。随后增强的启动审计、中文 HTTP、指标断言分别重跑通过。
- `cargo clippy --all-targets --locked --offline -- -D warnings`：零警告。
- `cargo fmt --check`：通过。
- `cargo build --release --locked --offline`：通过。
- `sample_memory.py` 本进程采样断言：Threads=1，RSS 为正，未把线程数换算为字节。

本轮不改前端，未重跑浏览器视觉验收；未运行目标 Ubuntu 24.04 systemd 实装、正式资料迁移、真实模型、Python/Rust 双向恢复或完整服务容量资格测试。P1/P2 代码修复不代表整份重构 Spec 或上线门槛已全部通过。工作树中原有 campus.db-shm/campus.db-wal 改动保留，未纳入任何提交。
