...
- `audience_scope`：管理员确认后的结构化条件，例如 `{"confirmed": true, "colleges": ["信息工程学院"], "entry_years": ["2024"]}`；空数组表示该维度不限，只有原文明确或管理员确认后才设置 `confirmed: true`。不能从不明确的自由文本猜测适用人群；未确认的未知标签会要求澄清。
